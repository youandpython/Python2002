...
def send_message_to_education():
    ...
    itchat.send('府前街小学{}月{}日无新增5类人员、无医学隔离和发热人员，《重点防控汇总-潍坊》数据无变化，《发热等特殊情况统计表》无新增人员。'
                .format(m, d), toUserName=room_id_education)
...
def send_message_to_school():
    ...
    for file in os.listdir(path):
        if os.path.isfile(os.path.join(path, file)) is True:
            new_name = file.replace(file, '{}月{}日图书电教实验共13人.rar'.format(m, d))
    ...
    if msg.User['NickName'] == room_name_class:
        if msg['ActualNickName'] == '数学老师' \
            ...
            if msg['Type'] == 'Recording':
                ...
                itchat.send(msg['User']['NickName']+'在'+timer + '发的语音：', toUserName=room_id_family)
                itchat.send('@%s@%s' % ('fil', msg['FileName']), toUserName=room_id_family)
            if msg['Type'] == 'Picture':
                msg['Text'](msg['FileName'])
                itchat.send(msg['User']['NickName']+'在'+timer + '发的图片：', toUserName=room_id_family)
                itchat.send('@%s@%s' % ('img', msg['FileName']), toUserName=room_id_family)
            if msg['Type'] == 'Video':
                msg['Text'](msg['FileName'])
                itchat.send(msg['User']['NickName']+'在'+timer + '发的视频：', toUserName=room_id_family)
                itchat.send('@%s@%s' % ('vid', msg['FileName']), toUserName=room_id_family)
            if msg['Type'] == 'Attachment':
                msg['Text'](msg['FileName'])
                itchat.send(msg['User']['NickName']+'在'+timer + '发的文件：', toUserName=room_id_family)
                itchat.send('@%s@%s' % ('fil', msg['FileName']), toUserName=room_id_family)
            if msg['Type'] == 'Text':
                if msg['FromUserName'] == itchat.get_friends(update=True)[0]['UserName']:
                    itchat.send('我在'+timer+'说：\n' + msg['Content'], toUserName=room_id_family)
                else:
                    itchat.send(msg['User']['NickName']+'在'+timer+'说：\n' + msg['Content'], toUserName=room_id_family)
            if msg['Type'] == 'Sharing':
                print('a sharing......')
                itchat.send("%s%s:\n%s\n%s" % (msg['User']['NickName']+'在'+timer + '分享的链接,', msg['Type'], msg['FileName'],
                                               msg['Url']), toUserName=room_id_family)

            if msg['Type'] == 'Map':
                print('a map......')
                itchat.send("%s%s:\n%s%s" % (msg['User']['NickName']+'在'+timer + '发的位置，', msg['Type'],
